...
export class BamlAsyncClient {
  ...
  async GradeActiveListening(
      scenario: string,history: types.Statement[],
    ...
    try {
      ...
      if (options.onTick) {
        const stream = this.stream.GradeActiveListening(
          scenario,history,
          __baml_options__
        );
      ...
      return raw.parsed(false) as types.Grade
    } catch (error) {
  ...
  async Talk(
      coach_characteristics: types.CoachCharacteristics,scenario: string,pretending: boolean,history: types.Statement[],
      __baml_options__?: BamlCallOptions
  ): Promise<string> {
    try {
      const options = { ...this.bamlOptions, ...(__baml_options__ || {}) }
      ...
      // Check if onTick is provided - route through streaming if so
      if (options.onTick) {
        const stream = this.stream.Talk(
          coach_characteristics,scenario,pretending,history,
          __baml_options__
        );
      ...
      const raw = await this.runtime.callFunction(
        "Talk",
        {
          "coach_characteristics": coach_characteristics,"scenario": scenario,"pretending": pretending,"history": history
        },
        this.ctxManager.cloneContext(),
        options.tb?.__tb(),
        options.clientRegistry,
