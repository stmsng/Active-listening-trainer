 ...
 *
 * **Return Type:**
 * - **Non‑streaming:** types.Grade
 * - **Streaming Partial:** Grade
 * - **Streaming Final:** types.Grade
 *
 * **Usage Patterns:**
 * 1. **Non‑streaming (Default)**
 *    - Best for quick responses and simple UI updates.
 ...
 * ```tsx
 * // Basic non‑streaming usage:
 * const { data, error, isLoading, mutate } = useGradeActiveListening({ stream: false});
 *
 * // Streaming usage:
 * const { data, streamData, isLoading, error, mutate } = useGradeActiveListening({
 *   stream: true | undefined,
 *   onStreamData: (partial) => console.log('Partial update:', partial),
 *   onFinalData: (final) => console.log('Final result:', final),
 *   onError: (err) => console.error('Error:', err),
 ...
 * ```
 */
export function useGradeActiveListening(props: HookInput<'GradeActiveListening', { stream: false }>): HookOutput<'GradeActiveListening', { stream: false }>
export function useGradeActiveListening(props?: HookInput<'GradeActiveListening', { stream?: true }>): HookOutput<'GradeActiveListening', { stream: true }>
export function useGradeActiveListening(
  props: HookInput<'GradeActiveListening', { stream?: boolean }> = {},
): HookOutput<'GradeActiveListening', { stream: true }> | HookOutput<'GradeActiveListening', { stream: false }> {
  let action: ServerAction = Actions.GradeActiveListening;
  if (isStreamingProps(props)) {
    action = StreamingActions.GradeActiveListening;
    return useBamlAction(action, props)
  } else {
    return useBamlAction(action, props as HookInput<'GradeActiveListening', { stream: false }>)
  }
}
/**
 * A specialized hook for the Talk BAML function that supports both streaming and non‑streaming responses.
 *
 * **Input Types:**
 *
 * - character_characteristics: types.CharacterCharacteristics
 ...
 *
 * **Return Type:**
 * - **Non‑streaming:** types.Update
 * - **Streaming Partial:** Update
 * - **Streaming Final:** types.Update
 *
 * **Usage Patterns:**
 * 1. **Non‑streaming (Default)**
 *    - Best for quick responses and simple UI updates.
 ...
 * ```tsx
 * // Basic non‑streaming usage:
 * const { data, error, isLoading, mutate } = useTalk({ stream: false});
 *
 * // Streaming usage:
 * const { data, streamData, isLoading, error, mutate } = useTalk({
 *   stream: true | undefined,
 *   onStreamData: (partial) => console.log('Partial update:', partial),
 *   onFinalData: (final) => console.log('Final result:', final),
 *   onError: (err) => console.error('Error:', err),
 ...
 * ```
 */
export function useTalk(props: HookInput<'Talk', { stream: false }>): HookOutput<'Talk', { stream: false }>
export function useTalk(props?: HookInput<'Talk', { stream?: true }>): HookOutput<'Talk', { stream: true }>
export function useTalk(
  props: HookInput<'Talk', { stream?: boolean }> = {},
): HookOutput<'Talk', { stream: true }> | HookOutput<'Talk', { stream: false }> {
  let action: ServerAction = Actions.Talk;
  if (isStreamingProps(props)) {
    action = StreamingActions.Talk;
    return useBamlAction(action, props)
  } else {
    return useBamlAction(action, props as HookInput<'Talk', { stream: false }>)
  }
}